...
def RunMain(low_dim=2, high_dim=25, initial_n=20, total_itr=100, func_type='Branin',
    ...
    if func_type=='Branin':
        ...
        test_func = functions.GrudLossFunc(active_var, noise_var=noise_var)
    else:
    ...
    if func_type == 'Rosenbrock':
        file_name = 'result/rosenbrock_results_CS_d' + str(low_dim) + '_D' + str(high_dim) + '_n' + str(
            initial_n)
    elif func_type == 'Branin':
        file_name = 'result/branin_results_CS_d' + str(low_dim) + '_D' + str(high_dim) + '_n' + str(
            initial_n)
    elif func_type == 'Hartmann6':
        file_name = 'result/hartmann6_results_CS_d' + str(low_dim) + '_D' + str(high_dim) + '_n' + str(
            initial_n)
    elif func_type == 'StybTang':
        file_name = 'result/stybtang_results_CS_d' + str(low_dim) + '_D' + str(high_dim) + '_n' + str(
            initial_n)
    elif func_type == 'WalkerSpeed':
        file_name = 'result/walkerspeed_results_CS_d' + str(low_dim) + '_D' + str(high_dim) + '_n' + str(
            initial_n)
    elif func_type == 'MNIST':
        file_name = 'result/mnist_results_CS_d' + str(low_dim) + '_D' + str(high_dim) + '_n' + str(
            initial_n)
    elif func_type == 'GRUD':
        file_name = 'result/grud_results_CS_d' + str(low_dim) + '_D' + str(high_dim) + '_n' + str(
            initial_n)
    else:
        raise
    ...
    for i in range(total_itr):
        ...
        save_dict = {'f_s': f_s_true, 'high_s': high_s}
